...
class Employee_Repository{
    ...
    int getHighestSalaryOfEmployees(){
        ...
        */
        System.out.println("Employee-Repository getHighestSalaryOfEmployees func");

        /* 
            Assuming we have SQL query capability, 
            query the database for the employee with the MAX salary from the Employee_Entity table
         */
        Employee_Entity employee;
        employee = databasaeConnection.sqlQuery.select("MAX(employee_entity.employee_salary)");
        if(employee != null){
            System.out.print("Highest Employee salary is: ");
            System.out.println("Highest Employee salary is: ");
            return employee.employee_salary;
        }
        else(
            System.out.println("Error finding top employee_salary");
            return 0;
        )
    }   
    ...
    string[] getTopTenHighestEarningEmployeeNames(){
        ...
        */
        ...
        System.out.println("Employee-Repository getTopTenHighestEarningEmployeeNames func");

        /* 
            Assuming we have SQL query capability, 
            query the database for employees from  the employee_entity table, 
            where employee_salary is sorted in descending order
         */
        string top_10_employee_names;
        Employee_Entity employee;
        employee = databasaeConnection.sqlQuery.selectAndOrderBy( "DESC(employee_entity.employee_salary)");
        if(employee != null){
            for(int i = 0; i < 10; i++){
                top_10_employee_names += employees[i].employee_name;
                top_10_employee_names += ", ";
            }
            System.out.print("Highest Employee salary is: ");
            System.out.println("Highest Employee salary is: ");
            return top_10_employee_names;
        }
    }
